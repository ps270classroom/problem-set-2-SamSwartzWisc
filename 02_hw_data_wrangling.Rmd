...
```
There are 2,509 challengers, 2,022 incumbents, and 2,448 open seats.
...
party_incumbent_promote <- filtered_fb_ads |>
  ...
  summarise(promote_prop = mean(ad_tone_promote, na.rm = TRUE))
knitr::kable(party_incumbent_promote, digits = 3, 
             col.names = c("Party", "Incumbency", "Promote Proportion"))
```
With a promote proportion of 0.854, Democratic incumbents sponsored the most promoting ads on average.
...
```{r}
fb_ads <- fb_ads |>
  mutate(impressions_millions = impressions / 1e6)
plot_q3 <- fb_ads |>
  filter(office == "US House") |>
  ggplot(aes(
    x = impressions_millions)) +
  geom_histogram()
plot_q3

```
The histogram is right-skewed meaning that there a more candidates that have lower impressions, specifically less than 10 million, and that frequency decreases as impressions increase.
...
fb_top_plot <- fb_ads |>
  ...
  ggplot(aes(x = impressions_millions, y = reorder(cand_name_last, impressions_millions))) + 
  geom_bar(stat = "identity", aes(fill = as.factor(party))) +
  labs(title = "Top 15 House Candidates by Facebook Ad Impressions",
       x = "Impressions (in millions)",
